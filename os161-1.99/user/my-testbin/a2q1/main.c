...
size_t SingleThreadedWordCount( struct Library * lib, char * word, int NUMTHREADS)
{
    size_t wordCount = 0;
...
typedef size_t (*counter)(struct Library *, char * word, int NUMTHREADS);

int CountOccurences( struct Library * lib, char * word, counter countFunc , double * time, int NUMTHREADS)
{
    struct timeval start, stop;
    gettimeofday(&start, NULL);
    size_t count = countFunc(lib, word, NUMTHREADS);
...
int main( int argc, char ** argv )
{
  ...
  return 0;
    ...
    int countSingle = CountOccurences( lib, word, SingleThreadedWordCount, &timeSingle, NUMTHREADS);
    int countMulti = CountOccurences( lib, word, MultithreadedWordCount, &timeMulti, NUMTHREADS);
